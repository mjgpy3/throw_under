...
describe Configurator do
  ...
  before(:each) do
    File.open('./config/config.tst.yaml', 'w') { |file| file.write(YAML.dump(config)) }
  end
  ...
  describe '#validate' do
    ...
    context 'when the config specifies multiple queues, missing inbound' do
      let(:config) { { 'queues' => [{ 'outbound' => 'foo', 'inbound' => 'bar.route_me' }, { 'outbound' => 'fizz' }] } }
    ...
    context 'when the config specifies multiple queues, missing outbound' do
      let(:config) { { 'queues' => [{ 'outbound' => 'foo', 'inbound' => 'bar.route_me' }, { 'inbound' => 'fizz' }] } }
    ...
    context 'when the config specifies both inbound and outbound queues' do
      let(:text_with_queues) { { 'queues' => [{ 'outbound' => 'foo', 'inbound' => 'bar.route_me' }] } }
      let(:config) { text_with_queues }

      it { is_expected.to be_an_instance_of(Configurator) }

      context 'and the config\' outbound queue does not have a "route_me" suffix' do
        let(:config) { { 'queues' => [{ 'outbound' => 'foo', 'inbound' => 'bar_no_suffix' }] } }

        it 'errors out because inbound queues must have a "route_me" suffix' do
          expect { subject }.to raise_error(RuntimeError, Configurator::INBOUND_SUFFIX_MISSING)
        end
      end
    ...
    context 'when the config\'s yaml has an inbound queue but not an outbound' do
      let(:config) { { 'queues' => [{ 'inbound' => 'foo.route_me' }] } }
